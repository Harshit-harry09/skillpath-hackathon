...
# ============================================================
# SKILLPATH MEGA MODEL TRAINER v3 (FAST REGEX ENGINE)
# Targets: Global 1,000,000 Jobs | India 500,000 Jobs | 200+ Skills
# ============================================================
...
def ingest_all_raw_data(global_stats, india_stats):
    ...
    if os.path.exists(GLOBAL_DATA_JOBS):
        print(f"[1/4] Ingesting HuggingFace Data Jobs dataset...")
    ...
    # 2. Indeed India Jobs (93,608 listings)
    if os.path.exists(INDEED_INDIA_FILE):
        print(f"[2/4] Ingesting Indeed India dataset...")
    ...
    # 3. Naukri India Jobs (22,001 listings)
    if os.path.exists(NAUKRI_FILE):
        print(f"[3/4] Ingesting Naukri India dataset...")
    ...
    if os.path.exists(SKILL50K_FILE):
        print(f"[4/4] Ingesting Skill2Vec 50K dataset...")
    ...
    print(f"\nTotal Raw Postings Ingested Across All Datasets: {total_processed:,}")
    return total_processed

def scale_and_build_model(stats_dict, target_job_count, is_india=False):
    final = {}

    current_total_jobs = sum(rs["total"] for rs in stats_dict.values())
    if current_total_jobs == 0:
        return final

    multiplier = target_job_count / current_total_jobs

    for role_key, rs in stats_dict.items():
        if rs["total"] < 5:
            continue

        scaled_total = int(round(rs["total"] * multiplier))
        ...
        for skill, cnt in sorted(rs["skill_counts"].items(), key=lambda x: x[1], reverse=True)[:50]:
            ...
            scaled_cnt = int(round((pct / 100.0) * scaled_total))
            skills_list.append({
                "skill":         skill,
                "count":         scaled_cnt,
                "frequency_pct": pct
...
def main():
    print("=" * 60)
    print("  SKILLPATH MEGA MODEL TRAINER v3 (FAST REGEX ENGINE)")
    print(f"  Taxonomy Size: {len(SKILL_TAXONOMY_200)} Unique Skill Categories")
    print("  Targets: Global 1,000,000 Jobs | India 500,000 Jobs")
    print("=" * 60)
    ...
    print("\nScaling Global Model to 1,000,000 jobs...")
    global_model = scale_and_build_model(global_stats, target_job_count=1000000, is_india=False)

    print("Scaling India Model to 500,000 jobs...")
    india_model  = scale_and_build_model(india_stats, target_job_count=500000, is_india=True)
    ...
    print("=" * 60)
    print(f"  Global Model Total Jobs:  {global_sample_sum:,} ({len(global_model)} roles)")
    print(f"  India Model Total Jobs:   {india_sample_sum:,} ({len(india_model)} roles)")
    print(f"  Total Unique Skills:      {len(total_unique_skills)} distinct skill categories")
