      ...
      <div className="max-w-[1280px] w-full">
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }} className="mb-12 md:mb-20 flex items-end justify-between flex-wrap gap-4">
          <div>
            ...
            <h2 className="font-comico font-normal uppercase leading-[0.98] tracking-wide" style={{ fontSize: 'clamp(36px, 5vw, 64px)', color: 'var(--color-ink)' }}>
        ...
        <div>
          {features.map((feat, i) => (
            ...
            >
              <div className="font-comico leading-none select-none flex-shrink-0" style={{ fontSize: 'clamp(64px, 8vw, 110px)', color: feat.accent, opacity: 0.15, lineHeight: 1, minWidth: '120px', textAlign: 'right' }}>
                {feat.num}
              </div>
              ...
              <div className="flex-1 space-y-3">
                <h3 className="font-comico font-normal uppercase leading-tight tracking-wide" style={{ fontSize: 'clamp(24px, 3.5vw, 36px)', color: 'var(--color-ink)' }}>{feat.title}</h3>
                <p className="font-zodiak text-[17px] font-normal leading-relaxed max-w-xl" style={{ color: 'var(--color-muted)' }}>{feat.desc}</p>
              </div>
