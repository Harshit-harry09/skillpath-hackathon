...
export function MarketToggle() {
  ...
  return (
    <button
      ...
      className="relative inline-flex h-9 w-9 items-center justify-center rounded-md border border-hairline bg-canvas text-ink transition-colors transition-transform duration-150 ease-out hover:bg-surface-soft hover:text-primary focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 cursor-pointer font-bold text-sm select-none shadow-sm active:scale-[0.97]"
    >
      <span className={`transition-transform transition-opacity duration-150 text-sm font-extrabold ${isIndia ? 'scale-100 opacity-100 text-brand-pink' : 'scale-90 opacity-0 absolute'}`}>
        ₹
      </span>
      <span className={`transition-transform transition-opacity duration-150 text-sm font-extrabold ${!isIndia ? 'scale-100 opacity-100 text-emerald-500' : 'scale-90 opacity-0 absolute'}`}>
        $
      </span>
    </button>
  );
