...
const DEFAULT_COLORS = ["#ff4d8b", "#2dd4bf", "#b8a4ed", "#ffb084", "#e8b94a"];
...
export type AppearFrom = "middle" | "top" | "bottom" | "left" | "right";
...
export interface PixelCardProps {
  ...
  style?: CSSProperties;
  className?: string;
  activeState?: "appear" | "disappear" | "idle";
}
...
export default function PixelCard(props: PixelCardProps) {
  ...
  const durationMs = (transition?.duration ?? 0.6) * 1000;
  ...
  useEffect(() => {
    initPixels();
    if (activeState === "appear") {
      handleAnimation("appear");
    } else if (activeState === "disappear") {
    ...
    const observer = new ResizeObserver(() => {
      initPixels();
      if (activeState) {
        handleAnimation(activeState === "disappear" ? "disappear" : "appear");
      }
    });
  ...
  return (
    <div
      ...
      className={className}
      style={rootStyle}
      onMouseEnter={activeState ? undefined : onMouseEnter}
      onMouseLeave={activeState ? undefined : onMouseLeave}
    >
      <canvas
        ref={canvasRef}
        style={{
